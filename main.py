...
# Configure logging to show everything in Railway logs
...
class Bot(BaseBot):
    ...
    async def on_chat(self, user: User, message: str) -> None:
        # Pass chat to commands.py
        await self.cmd.execute(user, message)
        # Update cache after command execution
        self.data_cache = self.cmd.load_data()

    async def on_user_join(self, user: User):
        # Brief sleep to prevent spam-kicks on join
        await asyncio.sleep(2)
